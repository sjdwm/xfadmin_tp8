...
class XiaoFeng
{
    ...
    public function __construct()
    {
        ...
        $this->WECHAT_SECRET = 'e865b1f514d';      // 微信小程序 AppSecret
                                                   // 头像保存路径（相对于项目根目录）
    ...
    public function wxlogin()
    {
        ...
        // 验证 code（必填）
        if (empty($data['code'])) {
            return json(['code' => 400, 'msg' => '缺少 code 参数', 'data' => '']);
        }
        ...
        if (! $response) {
            return json(['code' => 500, 'msg' => '微信服务器请求失败', 'data' => '']);
        }
        ...
        if (isset($wxData['errcode'])) {
            ...
            return json(['code' => 400, 'msg' => '微信登录失败: ' . $errMsg, 'data' => '']);
        }
        ...
        if (empty($openid)) {
            return json(['code' => 400, 'msg' => '获取 openid 失败', 'data' => '']);
        }
        ...
        if ($user_info) {
            // 用户已存在，更新登录信息
            $ip = request()->ip();
            Db::name('users')->where(['id' => $user_info['id']])->save([
                'login_time' => time(),
                'login_ip'   => $ip,
            ]);
 ...
 */
    public function login()
    {
        ...
        if (abs(time() - $data['timestamp']) > 300) {
            return json(['code' => 400, 'msg' => '请求已过期', 'data' => '']);
        }
        // 验证手机号（必填）
        if (empty($data['phone'])) {
        ...
        if (! preg_match('/^1[3-9]\d{9}$/', $data['phone'])) {
            return json(['code' => 400, 'msg' => '手机号格式不正确', 'data' => '']);

        }
        //使用手机号查询用户信息
        $user_info = Db::name('users')->where(['phone' => $data['phone']])->find();
        if (! $user_info > 0) {
            return json(['code' => 400, 'msg' => '该手机号未注册', 'data' => '']);
        }
        //判断账号是否锁
        if ($user_info['black_time'] + 60 > time()) {
            $s = ($user_info['black_time'] + 60) - time();
            return json(['code' => 400, 'msg' => "该账号已锁，请 {$s} 秒后再试！", 'data' => '']);
        }
        // 可以空密码登录,不为空就验证密码
        if (! empty($user_info['password'])) {
            ...
            if ($user_info['password'] != password($data['password'])) {
                ...
                if ($user_info['lock'] < 5) {
                    $s = 5 - $user_info['lock'];
                    Db::name("users")->where(['id' => $user_info['id']])->inc('lock', 1)->save();
                    return json(['code' => 400, 'msg' => '密码错误！您还有' . $s . '次机会', 'data' => '']);
                } else {
                    userLog('后台登录失败5次,用户名:' . $data['phone'], 4);
                    Db::name("users")->where(['id' => $user_info['id']])->save(['black_time' => time(), 'lock' => 0]);
                    return json(['code' => 400, 'msg' => '您的账号已被锁，请一分钟后再试！', 'data' => '']);
                }
            }
        ...
        //修改登录时间和IP,在线状态
        $ip = request()->ip();
        Db::name("users")->where(['id' => $user_info['id']])->save(['login_time' => time(), 'login_ip' => $ip]);
 ...
 */
    public function register()
    {
        ...
        if (empty($da['deviceId'])) {
            return json(['code' => 400, 'msg' => $da['deviceId'], 'data' => '']);
        } else {
            ...
            if (Db::name('users')->where("device_id", $da['deviceId'])->count() > 5) {
                return json(['code' => 400, 'msg' => '当前设备ID已经注册了5个账号,不可再注册了！', 'data' => '']);
            }
        ...
        if (Db::name('users')->where("login_ip", $ip)->count() > 5) {
            return json(['code' => 400, 'msg' => '当前IP已经注册了5个账号,不可再注册了！', 'data' => '']);
        }
        // 昵称长度限制
        if (mb_strlen($da['nickname']) > 20) {
            return json(['code' => 400, 'msg' => '昵称不能超过20个字符', 'data' => '']);
        }
        ...
        if (! empty($password)) {
            if (strlen($da['password']) < 6) {
                return json(['code' => 400, 'msg' => '密码长度不能少于6位', 'data' => '']);
            }
        }
        //注册账号处理
        ...
        if (Db::name('users')->where("phone", $da['phone'])->count()) {
            return json(['code' => 400, 'msg' => '手机号已经注册过了！', 'data' => '']);
        }
        $data['reg_time'] = time();
 ...
 */
    public function refresh()
    {
        ...
        if (empty($data['refreshToken'])) {
            return json(['code' => 400, 'msg' => 'Refresh Token 不能为空', 'data' => '']);
        }
        ...
        try {
            ...
            // 从数据库查询最新用户信息
            $user_info = Db::name('users')->where(['id' => $decoded->userId])->find();
    ...
    public function info()
    {
        ...
        if (! $authUser) {
            return json(['code' => 401, 'msg' => '未授权，请先登录', 'data' => '']);
        }
        ...
        // 查询用户信息
        $user_info = Db::name('users')->where(['id' => $userId])->find();
    ...
    public function update()
    {
        ...
        if (isset($data['phone']) && ! empty($data['phone'])) {
            ...
            // 检查手机号是否已被其他用户使用
            $existingUser = Db::name('users')->where(['phone' => $data['phone']])->where('id', '<>', $userId)->find();
            if ($existingUser) {
                return json(['code' => 400, 'msg' => '该手机号已被其他用户使用', 'data' => '']);
            }
        ...
        if (isset($data['birthday']) && ! empty($data['birthday'])) {
            ...
            if (! preg_match('/^\d{4}-\d{2}-\d{2}$/', $data['birthday'])) {
                return json(['code' => 400, 'msg' => '生日格式不正确，应为 YYYY-MM-DD', 'data' => '']);
            }
        ...
        // 如果没有需要更新的字段
        if (empty($updateData)) {
            return json(['code' => 400, 'msg' => '没有需要更新的字段', 'data' => '']);
        }

        // 执行更新
        $result = Db::name('users')->where(['id' => $userId])->save($updateData);

        if ($result !== false) {
            // 查询更新后的用户信息
            $updatedUser = Db::name('users')->where(['id' => $userId])->find();
            ...
            return json(['code' => 500, 'msg' => '更新失败', 'data' => '']);
        }
    }
    ...
    public function upload()
    {
        ...
        if (! $file) {
            return json(['code' => 400, 'msg' => '请选择要上传的文件', 'data' => '']);
        }
        ...
        if (! in_array($mimeType, $allowedTypes)) {
            return json(['code' => 400, 'msg' => '只支持上传 jpg、png、gif 格式的图片', 'data' => '']);
        }

                                    // 验证文件大小（最大 5MB）
        $maxSize = 5 * 1024 * 1024; // 5MB
        if ($file->getSize() > $maxSize) {
            return json(['code' => 400, 'msg' => '图片大小不能超过 5MB', 'data' => '']);
        }
        ...
        try {
            ...
            if (! is_dir($uploadDir)) {
                if (! mkdir($uploadDir, 0755, true)) {
                    return json(['code' => 500, 'msg' => '上传目录创建失败', 'data' => '']);
                }
            }
