...
function getTuzki(obj){
    ...
    if(tuzkiNumber){
        ...
        for (var i = 1; i < 41; i++) {
            ...
            str+='<img src="http://'+window.location.host+'/Public/emote/tuzki/t_'+number+'.gif" title="'+alt[i-1]+'" alt="MIXIM-EIP">';
        };
        tuzkiObj.html(str);
        tuzkiNumber=0;
    }else{
        ...
        if(data==1){
            ...
            if(content!='' && content!='Comment'){
                ...
                $.post(ajaxCommentUrl, postData, function(data) {
                    ...
                    var headImg=$('#b-login-word .b-head-img').attr('src');
                    var nickName=$('#username').val();
                    if(pid==0){
                        // pid为0表示新增评论
                        var str='<div class="row b-user b-parent"><div class="col-xs-2 col-sm-1 col-md-1 col-lg-1 b-pic-col"><img title="MIXIM-EIP" alt="MIXIM-EIP" src="'+headImg+'" class="b-user-pic" style="max-width: 121%;"></div><div class="col-xs-10 col-sm-11 col-md-11 col-lg-11 b-content-col"><p class="b-content"><span class="b-user-name">'+nickName+'</span>：'+content+'</p><p class="b-date">'+date+' <a onclick="reply(this)" username="'+nickName+'" pid="'+newPid+'" aid="'+aid+'" href="javascript:;">Reply</a></p><div class="b-clear-float"></div></div></div>';
                        $('.b-user-comment').prepend(str);
                    }else{
                        // pid不为0表示是回复评论
                        var str='<div class="row b-user b-child"><div class="col-xs-2 col-sm-1 col-md-1 col-lg-1 b-pic-col"><img title="MIXIM-EIP" alt="MIXIM-EIP" src="'+headImg+'" class="b-user-pic" style="max-width: 140%;"></div><ul class="col-xs-10 col-sm-11 col-md-11 col-lg-11 b-content-col"><li class="b-content"><span class="b-reply-name">'+nickName+'</span><span class="b-reply">Reply</span><span class="b-user-name">'+replyName+'</span>：'+content+'</li><li class="b-date">'+date+' <a onclick="reply(this)" pid="'+newPid+'" aid="'+aid+'" username="'+replyName+'" href="javascript:;">Reply</a></li><li class="b-clear-float"></li></ul></div>';
...
function reply(obj){
    ...
    var str='<div class="b-box-textarea"><div class="b-box-content" contenteditable="true" onfocus="delete_hint(this)">Reply to comments</div><ul class="b-emote-submit"><li class="b-emote"><i class="fa fa-smile-o" onclick="getTuzki(this)"></i><input class="form-control b-email" type="text" name="email" placeholder="接收回复的email地址" value="'+userEmail+'"><div class="b-tuzki"></div></li><li class="b-submit-button"><input type="button" value="Comment" aid="'+aid+'" pid="'+pid+'" username="'+username+'" onclick="comment(this)"></li><li class="b-clear-float"></li></ul></div>';
...
function delete_hint(obj){
    var word=$(obj).text();
    if(word=='Comment' || word=='Reply to comments'){
        $(obj).text('');
        $(obj).css('color', '#333');
    }
}
