                                ...
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
                                        <tr id="c<?php echo htmlentities($val['id']); ?>" c<?php echo htmlentities($val['id']); ?>="0"  onclick="show(<?php if($val['pid'] == '0'): ?><?php echo htmlentities($val['id']); ?><?php endif; ?>);" class="xshow <?php if($val['pid'] != '0'): ?>cshow<?php endif; ?>"> 
                                            ...
                                            <td><?php if($val['pid'] == '0'): ?><i id="d<?php echo htmlentities($val['id']); ?>" class="menu-icon fa fa-angle-right biao"></i><?php endif; ?> &nbsp;<?php echo htmlentities($val['_name']); ?></td>
                                            <td><?php echo htmlentities($val['ename']); ?></td>
                                            <td style="width:280px"><input name="o" style="width:280px" class="email" type="text" value="<?php echo htmlentities($val['email']); ?>"
                                                         val="<?php echo htmlentities($val['id']); ?>"/></td>
                                            <td style="width:80px"><input name="o" style="width:280px" class="bm" type="text" value="<?php echo htmlentities($val['bm']); ?>"
                                                         val="<?php echo htmlentities($val['id']); ?>"/></td>
                                            <!-- <td><?php echo htmlentities($val['address']); ?></td> -->
                                            ...
                                            <td><input name="o" class="inputorder" type="number" value="<?php echo htmlentities($val['o']); ?>"
                                                         val="<?php echo htmlentities($val['id']); ?>"/></td>
                                            <td class="center">
                                            <a class="green" href="<?php echo url('add',array('pid'=>$val['id'])); ?>"
                                                     title="新增分类"><i class="ace-icon fa fa-plus-circle bigger-100"></i>新增</a>&nbsp;&nbsp;
                                            <a class="blue" href="<?php echo url('edit',array('id'=>$val['id'])); ?>"><i class="ace-icon fa fa-plus-circle bigger-100"></i>修改</a>&nbsp;<a
                                                    class="red del" href="javascript:;" val="<?php echo url('del',array('ids'=>$val['id'])); ?>"
...
<script type="text/javascript">
    $(function () {
         $(".inputorder").change(function () {
            ...
            var o = $(this).val();
            if (id != o) {
                $.post("<?php echo url('add',array('act'=>'order')); ?>", {id: id, o: o}, function (data) {
                    if (data == 1) {
        ...
        $(".email").change(function () {
            ...
            var o = $(this).val();
            if (id != o) {
                $.post("<?php echo url('add',array('act'=>'email')); ?>", {id: id, o: o}, function (data) {
                    if (data == 1) {
                        window.location.reload(true);
                    }
                }
        ...
        $(".bm").change(function () {
            ...
            var o = $(this).val();
            if (id != o) {
                $.post("<?php echo url('add',array('act'=>'bm')); ?>", {id: id, o: o}, function (data) {
                    if (data == 1) {
                        window.location.reload(true);
                    }
                }
    ...
    function show(a){
        if(a){
            //s=0;
            $.ajax({
            url:"<?php echo url('company/is_show'); ?>",
            data:{'pid':a},
            type:'get',
