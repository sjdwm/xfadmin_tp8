                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                             <?php if(is_array($category) || $category instanceof \think\Collection || $category instanceof \think\Paginator): if( count($category)==0 ) : echo "" ;else: foreach($category as $key=>$val): ?>
                        <tr>
                            <td><?php echo $val['_name']; ?> </td>
                            <!-- <td><?php echo htmlentities($v['name']); ?></td> -->
                            <td><?php echo $val['dir']=='' ? htmlentities($val['url']) : htmlentities($val['dir']); ?></td>
                             <td>
                            <input name="o" class="inputorder" type="number" value="<?php echo htmlentities($val['o']); ?>"
                                                         val="<?php echo htmlentities($val['id']); ?>"/>
                                          
                            </td>
                            <td>
                                <a class="green" href="<?php echo url('add',array('pid'=>$val['id'])); ?>"
                                                     title="新增分类"><i class="ace-icon fa fa-plus-circle bigger-100"></i>新增</a>&nbsp;&nbsp;
                                <a class="blue" href="<?php echo url('edit',array('id'=>$val['id'])); ?>"
                                                     title="编辑分类"><i class="ace-icon fa fa-plus-circle bigger-100"></i>编辑</a>&nbsp;&nbsp;
                                <a class="red del" href="javascript:void(0);" val="<?php echo htmlentities($val['id']); ?>"
                                                     title="删除分类"><i class="ace-icon fa fa-plus-circle bigger-100"></i>删除</a>
                                            
                            </td>
                           <!--  <td>
                                <a href="javascript:;" ruleId="<?php echo htmlentities($v['id']); ?>" onclick="add_child(this)">添加子权限</a> |
                                <a href="javascript:;" ruleId="<?php echo htmlentities($v['id']); ?>" ruleName="<?php echo htmlentities($v['name']); ?>" ruleTitle="<?php echo htmlentities($v['title']); ?>" onclick="edit(this)">修改</a> |
                                <a href="javascript:if(confirm('确定删除？'))location='<?php echo url('Admin/Rule/delete',array('id'=>$v['id'])); ?>'">删除</a>
                            </td> -->
                        </tr>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">
    $(function () {
        $(".inputorder").change(function () {
            ...
            var o = $(this).val();
            if (id != o) {
                $.post("<?php echo url('update',array('act'=>'order')); ?>", {id: id, o: o}, function (data) {
                    if (data == 1) {
        ...
        $(".del").on('click', function () {
            ...
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result) {
                        $.get("<?php echo url('del'); ?>?id=" + id, function (result) {
                            if (result == 2) {
                                bootbox.alert({
                                    buttons: {
                                        ok: {
